...
class CalCFrame extends JFrame implements ActionListener
{
	...
	private String[] buttonText = { " 1 ", " 2 ", " 3 ", " + ", " - ", " 4 ", " 5 ", " 6 ", " x ", " / ", " 7 ", " 8 ", " 9 ", "^ ", "sqrt", " C ", " 0 ", " . ", "    =    " };
	...
	private static final int ADD = 1, // integer constants representing operators
			SUB = 2, MULT = 3, DIVI = 4, POW = 5, SQRT = 6;
	...
	static class WindowAdapterInner extends WindowAdapter
	{
		@Override
	...
	public CalCFrame(String title)
	{
		...
		*/
		addWindowListener(new WindowAdapterInner());
...
//==============================================================================
	public void actionPerformed(ActionEvent e)
	{
		...
		if (e.getSource() != buttons[3] && e.getSource() != buttons[4] && e.getSource() != buttons[8] && e.getSource() != buttons[9] && e.getSource() != buttons[13] && e.getSource() != buttons[14] && e.getSource() != buttons[15] && e.getSource() != buttons[18])
		{

			if (clearscreen)
			{ // clears screen if user enters number before an
...
//==============================================================================  
	public void processNumbers()
	{
		...
		// the program enters this "if" block when an operator is pressed for
		// the
		// first time
		if (firstpress)
		{
			...
			if (equals)
			{
				num1 = answer; // answer is stored in num1 if user enters equal
								// operator
				equals = false; // equals is set to false to allow additional
								// input
			} // end if
			else
				//num1 = Double.valueOf(input).doubleValue(); // converts a string
															// number to double
				num1=Double.parseDouble(input);

			oldoper = oper; // store current operator to oldoper

			// if operator is square root, calculation and output is done
			// immediately
			if (oper == SQRT)
			{
		...
		// "if" block is entered if now more than two numbers are being entered
		// to
		// be calculated
		else if (!morenums)
		{

			//num2 = Double.valueOf(input).doubleValue(); // converts second num
														// to double
			num2=Double.parseDouble(input);
			answer = calculate(oldoper, num1, num2); // calculate num1 and num2
														// with
			showAnswer(Double.toString(answer)); // the past operator
			newoper = oper; // store current operator to
							// new oper
			if (!equals)
				morenums = true; // tells program that more than two numbers
									// have
			else
			{ // entered
		...
		// if more than two numbers are being inputted to calculate, this "if"
		// block
		// is accessed
		else
		{
			...
			if (equals)
			{
				...
				newoper = oper;
				morenums = false;
				firstpress = true; // if equals is pressed set firstpress to
									// false
			} // end if
